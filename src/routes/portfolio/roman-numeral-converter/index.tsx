...
export default component$(() => {
  ...
  const conversion = useComputed$(() =>
    evaluateDecimalInput(decimalInput.value),
  );
  ...
  const handleSubmit = $((event: Event) => {
    ...
    history.value = saveConversion(history.value, {
    ...
    <div class="converter-page layout-shell">
      <section class="hero-card">
        <div class="hero-intro">
          <p class="text-xs font-semibold tracking-[0.38em] text-[var(--primary)] uppercase">
            Roman Numeral Converter
          </p>
          <h1>Quickly translate decimal numbers into classical numerals.</h1>
          <p>
            A responsive utility inspired by coding interview warmups. Type any
            whole number between 1 and 3,999 to see the additive and subtractive
            notation rendered instantly, then stash your favourite conversions
            for later reference.
          </p>
        </div>
        ...
        <div class="converter-shell">
          <article class="converter-card">
            <header>
              ...
              <p>
                Preview the numeral as you type. Submit the form to add the
                translation to a personal history log, perfect for preparing
                slides, puzzles, or kata practice.
              </p>
            </header>
          ...
          <article class="converter-card">
            <header>
              ...
              <p>
                The converter stores your six most recent submissions,
                deduplicated by decimal value. Each entry includes a timestamp
                from your current locale.
              </p>
            </header>
                ...
                {history.value.map((entry) => (
                  <li key={entry.decimal} class="history-item">
                    <div class="history-item-header">
                      <span aria-label={`Decimal value ${entry.decimal}`}>
                        {entry.decimal}
                      </span>
                    ...
                    <strong aria-label={`Roman numeral ${entry.roman}`}>
                      {entry.roman}
                    </strong>
                  </li>
                ))}
      ...
      <aside class="legend-card">
        <header>
          ...
          <p class="text-sm text-[var(--text3)]">
            The converter uses a greedy algorithm built on standard subtractive
            pairs. Each symbol is paired with the decimal value it represents.
          </p>
        </header>
