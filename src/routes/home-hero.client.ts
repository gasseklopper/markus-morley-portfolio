...
const createPatterns = (config: ReturnType<typeof createConfig>) =>
  ({
    flame: {
      name: "Flame Trail",
...
export const setupHomeHeroAnimations = async () => {
  ...
  const effectLinks = Array.from(document.querySelectorAll<HTMLAnchorElement>("[data-effect]"));
  ...
  effectLinks.forEach((link) => cleanupBag.add(on(link, "click", onEffectClick)));
  ...
  cleanupBag.add(on(heroSection, "touchstart", onTouchStart, { passive: true }));
  ...
  cleanupBag.add(on(document, "touchstart", onDocumentTouchStart, { passive: true }));
